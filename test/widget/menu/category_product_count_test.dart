...
void main() {
  setUp(() async {
    ...
    await repo.addProduct(
      Product(name: 'Iced Latte', basePrice: 3.5, category: coffee),
    );
    await repo.addProduct(
      Product(name: 'Cappuccino', basePrice: 4.0, category: coffee),
    );
  });

  testWidgets('CategoryPage updates item count when product is added', (
    WidgetTester tester,
  ) async {
    // 1. Pump CategoryPage
    await tester.pumpWidget(
      const MaterialApp(home: Scaffold(body: CategoryPage())),
    );
    ...
    // CategoryItemCard uses a Dismissible with a ValueKey(category.id)
    final repo = MenuRepository();
    final coffeeCategory = repo.categories.firstWhere(
      (c) => c.name == 'Coffee',
    );
    final coffeeCardFinder = find.byKey(ValueKey(coffeeCategory.id));
    ...
    await repo.addProduct(
      Product(name: 'Espresso', basePrice: 2.0, category: coffeeCategory),
    );
    ...
    expect(
      repo.products.where((p) => p.category?.id == coffeeCategory.id).length,
      3,
    );
